    ...
    <div class="card-body">
        <form method="get" class="row g-2 align-items-end">
            ...
            <div class="col-md-6">
                ...
                <a href="<?= site_url('admin/stone-inventory/returns') ?>" class="btn btn-light">Reset</a>
            </div>
        </form>
    </div>
</div>
                ...
                <tbody>
                    ...
                    <?php foreach (($returns ?? []) as $return): ?>
                        <tr>
                            ...
                            <td><?= (int) ($return['line_count'] ?? 0) ?></td>
                            <td><?= number_format((float) ($return['total_qty'] ?? 0), 3) ?></td>
                            <td><?= number_format((float) ($return['total_value'] ?? 0), 2) ?></td>
                            <td>
                                <div class="d-flex gap-1">
                                    <a href="<?= site_url('admin/stone-inventory/returns/view/' . $return['id']) ?>" class="btn btn-sm btn-outline-primary"><i class="fe fe-eye"></i></a>
                                    <a href="<?= site_url('admin/stone-inventory/returns/receipt/' . $return['id']) ?>" class="btn btn-sm btn-outline-success" target="_blank"><i class="fe fe-printer"></i></a>
                                    <a href="<?= site_url('admin/stone-inventory/returns/' . $return['id'] . '/edit') ?>" class="btn btn-sm btn-outline-info"><i class="fe fe-edit"></i></a>
                                    <form method="post" action="<?= site_url('admin/stone-inventory/returns/' . $return['id'] . '/delete') ?>" onsubmit="return confirm('Delete this return?');">
                                        <?= csrf_field() ?>
