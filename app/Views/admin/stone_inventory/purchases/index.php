    ...
    <div class="card-body">
        <form method="get" class="row g-2 align-items-end">
            ...
            <div class="col-md-6">
                ...
                <a href="<?= site_url('admin/stone-inventory/purchases') ?>" class="btn btn-light">Reset</a>
            </div>
        </form>
    </div>
</div>
                ...
                <tbody>
                    ...
                    <?php foreach (($purchases ?? []) as $purchase): ?>
                        <tr>
                            ...
                            <td><?= (int) ($purchase['line_count'] ?? 0) ?></td>
                            <td><?= number_format((float) ($purchase['total_qty'] ?? 0), 3) ?></td>
                            <td><?= number_format((float) ($purchase['total_value'] ?? 0), 2) ?></td>
                            <td><?= number_format((float) ($purchase['invoice_total'] ?? $purchase['total_value'] ?? 0), 2) ?></td>
                            <td>
                                <div class="d-flex gap-1">
                                    <a href="<?= site_url('admin/stone-inventory/purchases/view/' . $purchase['id']) ?>" class="btn btn-sm btn-outline-primary"><i class="fe fe-eye"></i></a>
                                    <a href="<?= site_url('admin/stone-inventory/purchases/' . $purchase['id'] . '/edit') ?>" class="btn btn-sm btn-outline-info"><i class="fe fe-edit"></i></a>
                                    <form method="post" action="<?= site_url('admin/stone-inventory/purchases/' . $purchase['id'] . '/delete') ?>" onsubmit="return confirm('Delete this purchase?');">
                                        <?= csrf_field() ?>
