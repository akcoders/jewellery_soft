...
class OrdersController extends BaseController
{
    public function index(): string
    {
        ...
        $orders = (new OrderModel())
            ->select('orders.id, orders.order_no, orders.order_type, orders.order_design_type, orders.status, orders.priority, orders.due_date, orders.created_at, cu.name AS sales_person_name')
            ->join('customer_users cu', 'cu.id = orders.sales_person_user_id', 'left')
            ->where('orders.customer_id', $customerId)
            ->orderBy('orders.id', 'DESC')->findAll();
        $salesPeople = (new CustomerUserModel())->where('customer_id', $customerId)
    ...
    public function create(): string
    {
        ...
        $salesPeople = (new CustomerUserModel())->where('customer_id', $customerId)
            ->where('role', 'sales_person')->where('is_active', 1)->orderBy('name', 'ASC')->findAll();
        $designs = db_connect()->table('design_masters')->select('id, design_code, name, category, subcategory')
