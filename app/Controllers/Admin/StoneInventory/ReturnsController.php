...
class ReturnsController extends BaseController
{
    ...
    public function index(): string
    {
        ...
        $builder = db_connect()->table('stone_inventory_return_headers rh')
            ->select('rh.*, ih.voucher_no as issue_voucher_no, k.name as karigar_name, COUNT(rl.id) as line_count, COALESCE(SUM(rl.qty), 0) as total_qty, COALESCE(SUM(rl.line_value), 0) as total_value', false)
    ...
    public function view(int $id)
    {
        $return = db_connect()->table('stone_inventory_return_headers rh')
            ->select('rh.*, ih.voucher_no as issue_voucher_no, ih.issue_date, k.name as karigar_name, iloc.name as location_name')
            ->join('stone_inventory_issue_headers ih', 'ih.id = rh.issue_id', 'left')
            ->join('karigars k', 'k.id = rh.karigar_id', 'left')
    ...
    {
        $return = db_connect()->table('stone_inventory_return_headers rh')
            ->select('rh.*, ih.voucher_no as issue_voucher_no, ih.issue_date, ih.issue_to, k.name as karigar_name, k.phone as karigar_phone, k.email as karigar_email, k.address as karigar_address, k.city as karigar_city, k.state as karigar_state, k.pincode as karigar_pincode, iloc.name as location_name')
            ->join('stone_inventory_issue_headers ih', 'ih.id = rh.issue_id', 'left')
            ->join('karigars k', 'k.id = rh.karigar_id', 'left')
            ->join('inventory_locations iloc', 'iloc.id = rh.location_id', 'left')
            ->where('rh.id', $id)
    ...
    public function update(int $id)
    {
        ...
        try {
            $db->transException(true)->transStart();
            (new KarigarMaterialAccountingService($db))->reverseHeaderVoucher('stone_inventory_return_headers', $id, 'Stone return updated', (int) session('admin_id'));
            ...
            (new KarigarMaterialAccountingService($db))->postInventoryHeader('stone', 'return', $id);
            $db->transComplete();
        } catch (Throwable $e) {
            $db->transRollback();
            return redirect()->back()->withInput()->with('error', $e->getMessage());
    ...
    public function delete(int $id)
    {
        ...
        try {
            $db->transException(true)->transStart();
            (new KarigarMaterialAccountingService($db))->reverseHeaderVoucher('stone_inventory_return_headers', $id, 'Stone return deleted', (int) session('admin_id'));
            $service->reverseReturn($id);
            $this->lineModel->where('return_id', $id)->delete();
    ...
    private function validateHeader(): ?string
    {
        ...
        $issueId = (int) $this->request->getPost('issue_id');
        if (! $this->resolveSelectedIssue($issueId)) {
    ...
    private function issueOptions(): array
    {
        return db_connect()->table('stone_inventory_issue_headers ih')
            ->select('ih.id, ih.issue_date, ih.voucher_no, ih.issue_to, ih.karigar_id, ih.location_id, k.name as karigar_name')
     ...
     */
    private function resolveSelectedIssue(int $issueId): ?array
    {
        if ($issueId <= 0) {
            return null;
        }
        ...
        $row = db_connect()->table('stone_inventory_issue_headers ih')
            ->select('ih.id, ih.karigar_id, ih.location_id, ih.issue_to, ih.voucher_no, ih.issue_date, k.name as karigar_name')
            ->join('karigars k', 'k.id = ih.karigar_id', 'left')
            ->where('ih.id', $issueId)
            ->get()
            ->getRowArray();
